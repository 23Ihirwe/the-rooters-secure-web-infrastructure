...
A production-style Linux infrastructure project demonstrating **NGINX reverse proxying and load balancing, HTTPS/TLS, UFW firewalling, two isolated backend services, and evidence-led troubleshooting**.
...
**The Rooters** built a small production-style web infrastructure on one Ubuntu Linux machine.

Two lightweight Python HTTP backends run locally on separate ports. **NGINX** is the single web entry point and acts as a **reverse proxy and load balancer**. The web entry point is protected with **HTTPS using a locally generated self-signed TLS certificate**, while **UFW** applies a least-exposure firewall policy.

The project also demonstrates evidence-led troubleshooting by deliberately stopping Backend 2, observing the failure, investigating the infrastructure, identifying the root cause, restoring the service, and verifying recovery.

The focus is **infrastructure configuration, security reasoning, testing, availability, and diagnostic reasoning**, rather than application complexity.
...
2. **UFW** permits only the required administration and web entry ports.
3. Secure traffic reaches **NGINX on port 443**.
4. NGINX terminates TLS and forwards requests to the backend pool.
5. Requests are distributed between `127.0.0.1:3001` and `127.0.0.1:3002`.
...
The two Python services return distinguishable responses so that load balancing can be proven.
...
Both services bind to `127.0.0.1`, keeping them on the local loopback interface.

---

## NGINX Reverse Proxy & Load Balancing

The NGINX upstream pool contains both backend services:
...
The NGINX configuration is validated before reloading:
...
Load balancing is verified using repeated HTTPS requests:

```bash
for i in {1..10}; do curl -sk https://localhost; echo; done
```

Responses from both Server 1 and Server 2 demonstrate that NGINX reaches both backend instances.
...
NGINX references the certificate and private key:
...
Certificate information can be inspected using:
...
NGINX redirects HTTP traffic to HTTPS:
...
The redirect is verified using:

```bash
curl -I http://localhost
```

The response shows `HTTP/1.1 301 Moved Permanently` and redirects the request to `https://localhost/`.
...
Ports `3001` and `3002` are absent from the public UFW allow rules, and the services bind to `127.0.0.1`.
...
Both backend ports appear again and repeated HTTPS requests return responses from both Server 1 and Server 2.

---

## Practical Evidence

The screenshots below were captured from the group's Ubuntu environment. Click any screenshot to open the full-size evidence.
...
Repeated HTTPS requests through NGINX return responses from both Server 1 and Server 2, demonstrating that requests are distributed across the backend pool.
...
UFW is active and permits the required entry ports `22`, `80`, and `443`. Backend ports `3001` and `3002` are not included in the public allow rules.

[![UFW firewall rules](the-rooters-secure-web-infrastructure/Evidence/04_firewall_rules.png)](the-rooters-secure-web-infrastructure/Evidence/04_firewall_rules.png)

### Troubleshooting Recovery

After Backend 2 is restored, both backend ports are listening again and repeated HTTPS requests can return responses from both servers.

[![Troubleshooting recovery](the-rooters-secure-web-infrastructure/Evidence/05_troubleshooting_recovery.png)](the-rooters-secure-web-infrastructure/Evidence/05_troubleshooting_recovery.png)
...
└── the-rooters-secure-web-infrastructure/
    ...
    │
    └── Evidence/
        ├── 01_backend_services.png
        ├── 02_load_balancing.png
        ├── 03_https_redirect.png
        ├── 04_firewall_rules.png
        └── 05_troubleshooting_recovery.png
```
...
The public certificate `the_rooters.crt` is different from the secret private key and may be included where required.
...
| Lovella | NGINX + Load Balancing | Proxy and load balancing |
| Elvire | SSL/TLS + HTTPS | Certificate, HTTPS and redirect |
| Hildegardine | Firewall + Troubleshooting | UFW and diagnosis/recovery |

Every member should understand the complete architecture even when leading a specific section.

---

## Technical Report & Demo

The repository supports the final **technical report**, **architecture diagram**, **Linux evidence**, NGINX/TLS/firewall configuration records, and the **recorded practical demonstration**.

The recording demonstrates the actual Linux environment and command execution.
